# 1 "C:\\Users\\felix\\OneDrive - Hans-Böckler-Berufskolleg\\Unterlagen\\FSE2A\\FSE2A_Zwischenprojekt\\SmartKick\\SmartKick.ino"
# 2 "C:\\Users\\felix\\OneDrive - Hans-Böckler-Berufskolleg\\Unterlagen\\FSE2A\\FSE2A_Zwischenprojekt\\SmartKick\\SmartKick.ino" 2
...
class RGBLED { // Klasse für RGB-LEDs
  ...
  public:
    RGBLED(byte redPin, byte greenPin, byte bluePin, byte gndPin) {
      ...
      pinMode(bluePin, 0x1);
      pinMode(gndPin, 0x1);
      digitalWrite(gndPin, 0x0); // GND-Pin auf LOW setzen
    }
...
void setup() {
  ...
  pinMode(pinArrayInput[0], 0x2); // Aktiviere interne Pull-up-Widerstände für die Eingänge
  pinMode(pinArrayInput[1], 0x2);

  attachInterrupt(((pinArrayInput[0]) == 2 ? 0 : ((pinArrayInput[0]) == 3 ? 1 : -1)), ISR_Goal1, 2); // Interrupts für Lichtschranken
  attachInterrupt(((pinArrayInput[1]) == 2 ? 0 : ((pinArrayInput[1]) == 3 ? 1 : -1)), ISR_Goal2, 2);

  setGameMode(2); // Setze den Spielmodus auf ClassicPlay
...
void loop() {
  ...
  checkGameEnd(); // Überprüfen, ob das Spiel beendet ist
}
