...
#define DEBUG_1_PRINT(x)     if(debugLevel >= 1) Serial.print(x)                                // Wichtige Meldungen (Level 1)
#define DEBUG_1_PRINTLN(x)   if(debugLevel >= 1) Serial.println(x)
#define DEBUG_1_PRINT_F(x)   if(debugLevel >= 1) Serial.print(F(x))
#define DEBUG_1_PRINTLN_F(x) if(debugLevel >= 1) Serial.println(F(x))

#define DEBUG_2_PRINT(x)     if(debugLevel >= 2) Serial.print(x)                                // Ausführliche Meldungen (Level 2)
#define DEBUG_2_PRINTLN(x)   if(debugLevel >= 2) Serial.println(x)
#define DEBUG_2_PRINT_F(x)   if(debugLevel >= 2) Serial.print(F(x))
#define DEBUG_2_PRINTLN_F(x) if(debugLevel >= 2) Serial.println(F(x))


byte pinArrayInput[2]{                                                                          // Array für Eingangs-Pins
    2,                                  // Lichtschranke Tor 1 (Interrupt)
    3                                   // Lichtschranke Tor 2 (Interrupt)
};

byte pinArrayOutput[4]{                                                                         // Array für RGB-LED-Pins
...
volatile unsigned long interruptData[4]{                                                        // Array für Interrupt-Daten
...
unsigned long scoreArray[6]{                                                                    // Array für Spielstand-Daten
    0,                                  // Spielstand Team 1
    0,                                  // Spielstand Team 2
...
byte configArray[4]{                                                                            // Array für Spielkonfiguration
...
String teamNames[2] = {                                                                         // Array für Teamnamen
    "Team Blau",                        // Name Team 1 (blau)
    "Team Gelb"                         // Name Team 2 (gelb)
};

const char* nextionTextFields[7] = {                                                            // Array für Nextion-Textfelder
  "t0.txt",                             // Überschrift
  "t1.txt",                             // Zeit
  "t2.txt",                             // Teamname 1
  "n0.val",                             // Spielstand 1
...
class RGBLED {                                                                                  // Klasse zur Steuerung der RGB-LED
  private:
  ...
  public:
    RGBLED(byte redPin, byte greenPin, byte bluePin, byte gndPin) {                            // Konstruktor mit Pin-Konfiguration
      this->redPin = redPin;
    ...
    void on(byte red, byte green, byte blue) {                                                  // LED mit Farbwerten einschalten
    ...
    void blink(int times, int duration, byte red, byte green, byte blue) {                      // LED-Blinkfunktion
      for (int i = 0; i < times; i++) {                                                         // Wiederholt die Anzahl der Male
...
bool debounce(volatile unsigned long &lastmillis) {                                             // Entprellfunktion für Interrupts
  if (millis() - lastmillis > interruptData[2]) {                                               // Prüft, ob Entprellzeit verstrichen ist
...
void ISR_Goal1() {                                                                              // Interrupt-Service-Routine für Tor 1
  if (configArray[3] == 1 && debounce(interruptData[0]) && scoreArray[3] == 0) {                // Prüft Spielstatus und Entprellung
...
void ISR_Goal2() {                                                                              // Interrupt-Service-Routine für Tor 2
  if (configArray[3] == 1 && debounce(interruptData[1]) && scoreArray[3] == 0) {                // Prüft Spielstatus und Entprellung
...
void printScore() {                                                                             // Funktion zur Ausgabe des Spielstands
  ...
  if (currentScoreString != previousScoreString || 
      ...
      interruptData[3] == 1) {                                                                  // Prüft, ob Update nötig ist
    
    DEBUG_1_PRINTLN_F("=== SPIELSTAND ===");                                                    // Überschrift ausgeben
    DEBUG_1_PRINT(teamNames[0]); DEBUG_1_PRINT_F(": "); DEBUG_1_PRINTLN(scoreArray[0]);         // Team 1 Punktestand
    DEBUG_1_PRINT(teamNames[1]); DEBUG_1_PRINT_F(": "); DEBUG_1_PRINTLN(scoreArray[1]);         // Team 2 Punktestand
    ...
    if (configArray[2] == 1) {                                                                  // Bei Zeitspiel verbleibende Zeit
      DEBUG_1_PRINT_F("Verbleibende Zeit: ");
    } else {                                                                                    // Sonst verstrichene Zeit
      DEBUG_1_PRINT_F("Verstrichene Zeit: ");
    }
    
    DEBUG_1_PRINT(minutes);                                                                     // Minuten ausgeben
    DEBUG_1_PRINT_F(":");                                                                       // Doppelpunkt ausgeben
    if (seconds < 10) DEBUG_1_PRINT_F("0");                                                     // Führende Null bei einstelligen Sekunden
...
void printConfig() {                                                                            // Konfiguration ausgeben
...
void initializeGameTime() {                                                                     // Spielzeit initialisieren
  scoreArray[4] = millis();                                                                     // Aktueller Zeitstempel speichern
  if (configArray[2] == 1 || configArray[2] == 2) {                                             // Bei Zeit- oder Klassischem Spiel
    scoreArray[5] = configArray[0] * 60;                                                        // Minuten in Sekunden umrechnen
  }
}

unsigned long getElapsedTime() {                                                                // Verstrichene Zeit berechnen
  return (millis() - scoreArray[4]) / 1000;                                                     // Zeit in Sekunden seit Spielbeginn
}

unsigned long getRemainingTime() {                                                              // Verbleibende Zeit berechnen
...
unsigned long calculateElapsedTime() {                                                          // Spielzeit je nach Modus berechnen
  if (scoreArray[3] == 1) {                                                                     // Bei Spielende
    return scoreArray[5];                                                                       // Endzeit zurückgeben
  } else if (configArray[3] == 1) {                                                             // Bei laufendem Spiel
    if (configArray[2] == 1) {                                                                  // Bei Zeitspiel-Modus
      return getRemainingTime();                                                                // Verbleibende Zeit
    } else {                                                                                    // Bei anderen Modi
      return getElapsedTime();                                                                  // Verstrichene Zeit
    }
  } else {                                                                                      // Bei gestopptem Spiel
    return 0;                                                                                   // Zeit ist 0
  }
}

void checkGameEnd() {                                                                           // Prüft, ob Spielende erreicht ist
  if (scoreArray[3] == 1) return;                                                               // Wenn Spiel bereits beendet, abbrechen

  switch (configArray[2]) {                                                                     // Je nach Spielmodus
    case 1: {                                                                                   // Zeit-Modus
      unsigned long remainingTime = getRemainingTime();                                         // Verbleibende Zeit berechnen
      if (remainingTime == 0) {                                                                 // Wenn keine Zeit mehr übrig
        scoreArray[3] = 1;                                                                      // Spiel ist beendet
      }
      break;
    }
    case 2: {                                                                                   // Klassischer Modus
      if (scoreArray[0] >= configArray[1] || scoreArray[1] >= configArray[1]) {                 // Wenn Siegtore erreicht
        scoreArray[3] = 1;                                                                      // Spiel ist beendet
      }
      break;
    }
  ...
  if (scoreArray[3] == 1) {                                                                     // Wenn Spielende erreicht
    stopGame();                                                                                 // Spiel stoppen
    scoreArray[5] = getElapsedTime();                                                           // Gesamtspielzeit speichern
    switch (scoreArray[0] > scoreArray[1] ? 1 : (scoreArray[1] > scoreArray[0] ? 2 : 0)) {      // Gewinner ermitteln
      case 1:                                                                                   // Team 1 hat gewonnen
        ...
        if (nextionEnabled) {sendCommandToNextion(String(nextionTextFields[1]) + "=\"" + teamNames[0] + " gewinnt!\"");}
        break;
      case 2:                                                                                   // Team 2 hat gewonnen
        ...
        if (nextionEnabled) {sendCommandToNextion(String(nextionTextFields[1]) + "=\"" + teamNames[1] + " gewinnt!\"");}
        break;
      case 0:                                                                                   // Unentschieden
        ...
        if (nextionEnabled) {sendCommandToNextion(String(nextionTextFields[1]) + "=\"Unentschieden!\"");}
        break;
    }
  }
}

void resetGame() {                                                                              // Spiel zurücksetzen
...
void startGame() {                                                                              // Spiel starten
  ...
  if (nextionEnabled) {                                                                         // Wenn Nextion aktiviert
    // Teamnamen senden
    ...
    switch (configArray[2]) {                                                                   // Je nach Modus
      case 0: currentGameMode = "FreePlay"; break;
      case 1: currentGameMode = "TimePlay"; break;
      case 2: currentGameMode = "ClassicPlay"; break;
      default: currentGameMode = "Unknown"; break;
    }
...
void processCommand(String input) {                                                             // Befehl verarbeiten
  ...
  if (firstSpace == -1) {                                                                       // Wenn kein Leerzeichen gefunden
  ...
  for (int i = 0; i < 4; i++) {                                                                 // Alle bekannten Befehlstypen durchsuchen
    if (cmdType == COMMAND_TYPES[i]) {                                                          // Wenn Typ gefunden
...
void handleGetCommand(String params) {                                                          // GET-Befehle verarbeiten
  DEBUG_2_PRINT_F("HandleGetCommand mit Parameter: ");
  DEBUG_2_PRINTLN(params);
  
  // GET-Befehl im Array suchen
  int cmdIndex = -1;                                                                            // Index für GET-Befehl
  for (int i = 0; i < 2; i++) {                                                                 // Alle bekannten GET-Befehle durchsuchen
    if (params == GET_COMMANDS[i]) {                                                            // Wenn Befehl gefunden
...
void handleSetCommand(String params) {                                                          // SET-Befehle verarbeiten
  ...
  if (space != -1) {                                                                            // Wenn Leerzeichen gefunden
  ...
  for (int i = 0; i < 12; i++) {                                                                // Alle bekannten SET-Befehle durchsuchen
    if (command == SET_COMMANDS[i]) {                                                           // Wenn Befehl gefunden
      cmdIndex = i;                                                                             // Index speichern
      break;                                                                                    // Schleife beenden
    }
  }
...
void handleNxtCommand(String params) {                                                          // NXT-Befehle verarbeiten
  if (nextionEnabled) {                                                                         // Wenn Nextion aktiviert
...
void handleHelpCommand(String params) {                                                         // HELP-Befehle verarbeiten
  if (params == "get") {                                                                        // Hilfe für GET-Befehle
  ...
  } else if (params == "set") {                                                                 // Hilfe für SET-Befehle
...
void handleSetMode(String value) {                                                              // Spielmodus setzen
  for (int i = 0; i < 3; i++) {                                                                 // Alle bekannten Modi durchsuchen
    if (value == MODE_VALUES[i]) {                                                              // Wenn Modus gefunden
...
void handleSetTime(String value) {                                                              // Spielzeit setzen
...
void handleSetGoals(String value) {                                                             // Tore zum Sieg setzen
...
void handleSetScore(int teamIndex, String value) {                                              // Spielstand setzen
  if (value == "+") {                                                                           // Wenn "+" als Wert
    if (scoreArray[teamIndex] < 99) {                                                           // Nur erhöhen, wenn unter 99
      scoreArray[teamIndex]++;                                                                  // Spielstand erhöhen
    }
  } else if (value == "-") {                                                                    // Wenn "-" als Wert
    if (scoreArray[teamIndex] > 0) {                                                            // Nur verringern, wenn über 0
      scoreArray[teamIndex]--;                                                                  // Spielstand verringern
    }
  } else {                                                                                      // Sonst (direkter Wert)
    int newScore = value.toInt();                                                               // String in Zahl umwandeln
    if (newScore < 0) {                                                                         // Wenn Wert negativ
      newScore = 0;                                                                             // Auf Minimum setzen
    } else if (newScore > 99) {                                                                 // Wenn Wert zu groß
...
void handleSetName(int teamIndex, String value) {                                               // Teamnamen setzen
...
void handleSetFlag(int flagIndex, String value) {                                               // Flag-Werte setzen
  if (flagIndex == 10) {                                                                        // Debug-Level
    int level = value.toInt();                                                                  // String in Zahl umwandeln
    
    if (level >= 0 && level <= 2) {                                                             // Wenn Level gültig
...
void sendCommandToNextion(const String &command) {                                              // Befehl an Nextion senden
  DEBUG_2_PRINT_F("Nextion: ");
  DEBUG_2_PRINTLN(command);
  
  if (nextionEnabled) {                                                                         // Wenn Nextion aktiviert
    nextionSerial.print(command);                                                               // Befehl senden
    for (uint8_t i = 0; i < 3; i++) {                                                           // 3x 0xFF als Abschluss senden
      nextionSerial.write(0xFF);
    }
  }
}

String readNextionResponse() {                                                                  // Antwort vom Nextion lesen
  ...
  while ((millis() - startTime) < 100) {                                                        // Timeout von 100ms
    if (nextionSerial.available()) {                                                            // Wenn Daten verfügbar
      ...
      if (response.endsWith("\xFF\xFF\xFF")) {                                                  // Wenn Ende-Marker gefunden
        break;                                                                                  // Schleife beenden
      }
    ...
    if ((millis() - startTime) > 50 && response.length() > 0) {                                 // Timeout für Antwort
      break;
    }
  }

  // Ignoriere Antworten mit Command-Codes
  if (response.length() >= 4 && response[0] >= 0x00 && response[0] <= 0x23 && 
      response.endsWith("\xFF\xFF\xFF")) {                                                      // System-Antworten ignorieren
    return "";
  }

  // Entferne die Endsequenz "\xFF\xFF\xFF"
  if (response.endsWith("\xFF\xFF\xFF")) {                                                      // Wenn Ende-Marker vorhanden
    response = response.substring(0, response.length() - 3);                                    // Marker entfernen
  }

  // Debug-Ausgabe der empfangenen Daten
  if (response.length() > 0) {                                                                  // Wenn Antwort nicht leer
...
void updateNextion() {                                                                          // Nextion-Display aktualisieren
  static String lastValues[6] = {                                                               // Zuletzt gesendete Werte
    "",                                  // 0: Zeit
    "",                                  // 1: Spielstand Team 1
    "",                                  // 2: Spielstand Team 2
    "",                                  // 3: Teamname 1
  ...
  // Zeit aktualisieren
  if (configArray[3] == 1) {                                                                    // Wenn Spiel läuft
    ...
    if (currentTime != lastValues[0]) {                                                         // Wenn Zeit geändert
      sendCommandToNextion(nextionTextFields[1] + String("=\"") + currentTime + "\"");          // Zeit senden
      lastValues[0] = currentTime;                                                              // Neue Zeit speichern
    }
  } else {                                                                                      // Wenn Spiel nicht läuft
    if (lastValues[0] != "00:00") {                                                             // Wenn Zeit nicht 00:00
      sendCommandToNextion(nextionTextFields[1] + String("=\"00:00\""));                        // Zeit auf 00:00 setzen
      lastValues[0] = "00:00";                                                                  // Neue Zeit speichern
    }
  }

  // Spielstand Team 1 aktualisieren
  if (String(scoreArray[0]) != lastValues[1]) {                                                 // Wenn Spielstand geändert
  ...
  if (String(scoreArray[1]) != lastValues[2]) {                                                 // Wenn Spielstand geändert
  ...
  if (teamNames[0] != lastValues[3]) {                                                          // Wenn Teamname geändert
  ...
  if (teamNames[1] != lastValues[4]) {                                                          // Wenn Teamname geändert
  ...
  if (currentGameMode != lastValues[5]) {                                                       // Wenn Spielmodus geändert
...
void setup() {                                                                                  // Setup-Funktion (wird einmal ausgeführt)
...
void loop() {                                                                                   // Hauptschleife (wird wiederholt ausgeführt)
  if (Serial.available() > 0) {                                                                 // Wenn Daten von serieller Schnittstelle
    delay(50);                                                                                  // Kurze Verzögerung für vollständige Daten
    
    String input = "";
    // Alle verfügbaren Zeichen lesen
    while (Serial.available() > 0) {                                                            // Solange Daten verfügbar
      char c = Serial.read();                                                                   // Zeichen lesen
      if ((c == '\r' || c == '\n') && input.length() == 0) {                                    // CR/LF am Anfang ignorieren
        continue;
      }
    ...
    if (input.length() > 0) {                                                                   // Wenn Input nicht leer
  ...
  if (nextionEnabled) {                                                                         // Wenn Nextion aktiviert
    if (nextionSerial.available() > 0) {                                                        // Wenn Daten vom Display
